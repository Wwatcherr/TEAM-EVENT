...
## 🆕 Si tu mets à jour une app déjà déployée (membres + commentaires)

Si ton app est déjà en ligne, exécute ce script dans le SQL Editor de Supabase (disponible aussi dans `supabase-migration-members-comments.sql`, déjà pré-rempli avec la liste de membres initiale) :
...
Une fois fait, à la connexion chacun choisit son prénom dans une liste déroulante (avec une option "Autre / nouveau membre" pour les nouveaux arrivants, qui s'ajoutent alors automatiquement à la liste). Tu peux gérer cette liste à tout moment en cliquant sur l'icône d'équipe en haut à droite de l'app (ajout/suppression de membres, sans toucher au code). Sur chaque événement, un compteur dépliable affiche qui n'a pas encore répondu, pratique pour relancer les retardataires. Et quand quelqu'un répond "Présent", un champ apparaît pour ajouter un commentaire (ex : "ramène des chips"), visible par toute l'équipe dans la liste des participants.
